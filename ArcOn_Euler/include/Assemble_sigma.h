      ...
      if ( cell->is_locally_owned()  )
	{
	  ...
	  for (unsigned int i=0; i<dofs_per_cell; ++i){
	    for (unsigned int q=0; q<n_q_points; ++q){
	      ...
	      //test
	      /* sigma_interior[component](i) +=  fe_values[*(sigma[component])].value(i,q)   */
	      /*  	* (soln_sigma[component][q]) * JxW[q]; */
	  ...
	  for(unsigned int face_num=0; face_num < GeometryInfo<dim>::faces_per_cell; ++face_num){
	    ...
	    if ( face->at_boundary() && (face->boundary_indicator() == 0) ){
	      ...
	      soln_sigma_face[component] =  std::vector<Tensor<1,dim> >(n_q_points_face);
	      fe_values_face[*(alpha[component])].get_function_gradients(subdomain_solution[component],
									 soln_sigma_face[component]);
		...
		wbv.value_list( quadrature_point, alphas_boundary, component, current_time);
		  ...
		  if (component == 2){
		    ...
		    //testing
		      /* sigma_flux[component](i) +=  fe_values_face[*(sigma[component])].value(i,q)   */
		      /* 	* (soln_sigma[component][q]) * JxW[q]; */
		...
		hp_fe_values_neigh_face[component]->get_present_fe_values();
	      ...
	      fe_values_face[*(alpha[component])].get_function_gradients(subdomain_solution[component],
									 soln_sigma_face[component]);
	      soln_sigma_neigh_face[component] =  std::vector<Tensor<1,dim> >(n_q_points_face);
	      fe_values_neigh_face[*(alpha[component])].get_function_gradients(subdomain_solution[component],
									       soln_sigma_neigh_face[component]);
