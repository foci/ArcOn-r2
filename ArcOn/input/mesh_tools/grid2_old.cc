...
// double nexty(const double x,const double y) const
// {  
//   //return std::fmod(y+ a1*std::sin(y) + 2.0*M_PI/(3.0+x),2.0*M_PI);
//   return y+a1*std::cos(y) + 2.0*M_PI/(3.0+x);
...
void laplace_grid ()
{
  ...
  for (Triangulation<2>::cell_iterator cell2 = tria.begin();
    ...
    for (unsigned int f=0; f<GeometryInfo<2>::faces_per_cell; ++f) {
      if (cell2->face(f)->at_boundary() && (cell2->face(f)->center()[0] == 6.0)) 
  	cell2->face(f)->set_boundary_indicator(1);
      if (cell2->face(f)->at_boundary() && (cell2->face(f)->center()[0] == 0.0)) 
  	cell2->face(f)->set_boundary_indicator(2);
      if (cell2->face(f)->at_boundary() && (cell2->face(f)->center()[1] == 0.0 || cell2->face(f)->center()[1] == 2.0*M_PI)) 
  ...
  for ( ; cell != endc; ++cell)
	...
	double x0 = cell->face(f)->vertex(v)[0];
	double y0 = cell->face(f)->vertex(v)[1];

	//new_points[v_n] = Point<2> (x0,y0);
      
	if (cell->face(f)->at_boundary() && cell->face(f)-> boundary_indicator() ==1)  {
	  double x1 = x0 + 1.0*std::sin(y0);
	  new_points[v_n] = Point<2> (x1,y0); 
	}
	else if (cell->face(f)->at_boundary() && cell->face(f)-> boundary_indicator() ==2)  {
	  double x1 = x0;
	  new_points[v_n] = Point<2> (x1,y0); 
	}
	else if (cell->face(f)->at_boundary() && cell->face(f)-> boundary_indicator() ==4 )  
	  new_points[v_n] = Point<2> (x0,y0); 
